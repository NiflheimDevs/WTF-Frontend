...
// ── Input Field ────────────────────────────────────────────────────
function Field({ label, type = 'text', value, onChange, placeholder, error, hint, children }) {
  return (
...
function Alert({ message, type = 'danger' }) {
  ...
  return (
    <div className={`px-3 py-2.5 rounded-md border text-sm font-medium ${styles[type]}`}>
      {message}
    </div>
  )
}
...
function LoginForm({ onSwitch }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Simulated failure for now — remove when backend is ready
      ...
      } else if (err.message === 'backend-needed') {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <Field
        ...
        type="email"
        value={email}
        onChange={val => { setEmail(val); setFieldErrors(e => ({ ...e, email: null })) }}
      ...
      <Field
        ...
        value={password}
        onChange={val => { setPassword(val); setFieldErrors(e => ({ ...e, password: null })) }}
        placeholder="••••••••"
...
function SignupForm({ onSwitch }) {
  ...
  const strength = (() => {
    ...
    if (password.length < 8)   return { level: 1, label: 'Too short', color: 'bg-danger-fg'  }
    if (password.length < 12)  return { level: 2, label: 'Fair',      color: 'bg-warning-fg' }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <Field
        label="Full name"
        value={name}
        onChange={val => { setName(val); setFieldErrors(e => ({ ...e, name: null })) }}
      ...
      <Field
        ...
        value={confirm}
        onChange={val => { setConfirm(val); setFieldErrors(e => ({ ...e, confirm: null })) }}
        placeholder="••••••••"
...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-neutral-0 flex flex-col items-center justify-center px-4 font-sans"
      style={{ background: 'radial-gradient(circle at 0% 0%, rgba(11,107,203,0.08), transparent 50%), var(--color-neutral-0)' }}
    >
      {/* Card */}
        ...
        <div className="flex flex-col items-start gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-500 mt-0.5">
              {tab === 'login' ? 'Sign in to coordinate relief' : 'Register as a new dispatcher'}
            </p>
          </div>
        </div>
